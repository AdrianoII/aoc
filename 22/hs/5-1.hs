...
applyMove :: Int -> Int -> [String] -> [String]
applyMove f t stack = concat $ before_temp : [f_col] : [after_temp]
  where
    pf = pred f
    pt = pred t
    (_, (e : f_col) : _) = splitAt pf stack
    (before_t, t_col : after_t) = splitAt pt stack
    temp = concat $ before_t : [e : t_col] : [after_t]
    (before_temp, _ : after_temp) = splitAt pf temp

applyMoves :: [[Int]] -> [String] -> [String]
applyMoves [] s = s
applyMoves ([n, f, t] : xs) s = applyMoves xs ((!! n) $ iterate (applyMove f t) s)
